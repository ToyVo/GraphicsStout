...
class Cube {
	...
	bufferData(gl) {
		...
		gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array([0, 1, 1, 1]));
		...
		img.onload = function() {
			...
			wgl.texImage2D(wgl.TEXTURE_2D, 0, wgl.RGBA, wgl.RGBA, wgl.UNSIGNED_BYTE, this);
	...
	render(gl, projection, view) {
		...
		const verLoc = gl.getAttribLocation(this.program, 'cubeLocation');
		const verCol = gl.getAttribLocation(this.program, 'cubeColor');
		...
		const texLoc = gl.getAttribLocation(this.program, 'textureCoord');
		const sample = gl.getUniformLocation(this.program, 'sample');
