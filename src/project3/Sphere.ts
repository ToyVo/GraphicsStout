	...
	constructor(numTimesToSubdivide) {
		...
		for (let divisionNumber = 1; divisionNumber < numTimesToSubdivide; divisionNumber++) {
			...
			for (let i = 0; i < vertices.length; i += 9) {
				const A = new Vector([vertices[i], vertices[i + 1], vertices[i + 2]]);
				const B = new Vector([vertices[i + 3], vertices[i + 4], vertices[i + 5]]);
				const C = new Vector([vertices[i + 6], vertices[i + 7], vertices[i + 8]]);
				const AB = findMidpoint(A, B);
				const AC = findMidpoint(A, C);
				const BC = findMidpoint(B, C);
				AB.normalize();
				AC.normalize();
				BC.normalize();
		...
		function findMidpoint(A, B) {
			const x = (A.getX() + B.getX()) / 2;
			const y = (A.getY() + B.getY()) / 2;
			const z = (A.getZ() + B.getZ()) / 2;
